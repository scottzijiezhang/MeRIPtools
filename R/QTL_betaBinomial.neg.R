...
#' @import broom
#' @export
swapChr22.QTL_BetaBin <- function( MeRIPdata , vcf_file, BSgenome = BSgenome.Hsapiens.UCSC.hg19,testWindow = 100000, Chromosome, Range = NULL, Covariates = NULL, AdjustGC = TRUE, AdjIPeffi = TRUE , PCsToInclude = 0 , thread = 1 ){
  ...
  if( AdjIPeffi ){ variables <- paste(variables, "offset(K_IPe_ij)", sep  = " +") }
...
#' @title swapChr1.QTL_BetaBin
#' @description Take the phenotype on chr2 and test their association with SNPs on chr1
#' @param MeRIPdata The MeRIP.Peak object
#' @param vcf_file The vcf file for genotype. The chromosome position must be sorted!!
#' @param BSgenome The BSgenome object. This needs to match the genome version of the gtf files. 
#' @param testWindow Integer. Test SNPs in <testWindow> bp window flanking the peak.
...
#' @import broom
#' @export
swapChr1.QTL_BetaBin <- function( MeRIPdata , vcf_file, BSgenome = BSgenome.Hsapiens.UCSC.hg19,testWindow = 100000, Chromosome, Range = NULL, Covariates = NULL, AdjustGC = TRUE, AdjIPeffi = TRUE , PCsToInclude = 0 , thread = 1 ){
  ...
  MeRIPdata@jointPeaks$chr <- gsub("chr1","chr111",MeRIPdata@jointPeaks$chr )
  MeRIPdata@jointPeaks$chr <- gsub("chr2","chr1",MeRIPdata@jointPeaks$chr )
  ## set ranges on the chromosome that can be tested
  con <- pipe(paste0("zcat ",vcf_file," | awk '!/^#/ {print $2}' | tail -n1"))
  vcfRange <- c( read.table(gzfile(vcf_file), nrows = 1)[,2] ,
                 scan( con , quiet = T ))
